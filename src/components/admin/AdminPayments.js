...
function AdminPayments() {
    ...
    const handleCreatePayment = async () => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.message || 'Failed to process payment.'; // Ambil pesan kesalahan dari backend
    ...
    const handleUpdatePayment = async () => {
        try {
            const response = await axios.put(`http://localhost:5000/api/payments/${editPayment.payment_id}`, editPayment);
            fetchPayments();
            setEditPayment(null);
            Swal.fire('Success!', response.data.message, 'success'); // Tampilkan pesan dari backend
        } catch (error) {
            console.error('Error updating payment:', error);
            const errorMessage = error.response?.data?.message || 'Failed to update payment status.'; // Ambil pesan kesalahan dari backend
            Swal.fire('Error!', errorMessage, 'error'); // Tampilkan pesan kesalahan
        }
    };
    ...
    const handleDeletePayment = async (paymentId) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                const errorMessage = error.response?.data?.message || 'Failed to delete payment.'; // Ambil pesan kesalahan dari backend
                Swal.fire('Error!', errorMessage, 'error'); // Tampilkan pesan kesalahan
            }
        }
    };
