    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-white flex flex-col items-center">
      ...
      <div className="flex justify-center gap-8 flex-wrap mb-8">
        {filteredShoes.map(shoe => (
          <div key={shoe.shoe_detail_id} className="bg-gray-800 rounded-lg p-4 text-center relative w-60 transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
            <Link to={`/shoes/${shoe.shoe_detail_id}`} onClick={() => handleShoeClick(shoe.shoe_detail_id)}>
              <img src={`/images/${shoe.shoe_name}.jpg`} alt={shoe.shoe_name} className="w-full h-auto rounded-lg mb-4" />
              <div className="mb-2">
                <span className="text-sm text-gray-400">{getCategoryName(shoe.category_id)}</span>
                <h3 className="text-lg font-bold">{shoe.shoe_name}</h3>
                <p className="text-md mt-1">{shoe.shoe_price.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' })}</p>
              </div>
            </Link>
